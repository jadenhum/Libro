...
const RecurringBookings = ({
  ...
  return (
    bookings.length !== 0 && (
      <>
        {bookings.length === 1 ? (
          <h1 className="text-1xl font-bold text-gray-500 mb-2 mt-10">Recurring Booking</h1>
        ) : (
          <h1 className="text-1xl font-bold text-gray-500 mb-2 mt-10">Recurring Bookings</h1>
        )}
          ...
          <table className="min-w-full table-fixed bg-white border border-gray-200">
            <thead>
              <tr className="text-left bg-gray-100">
                ...
                <th className="py-2 px-4 border-b">Begins On</th>
                <th className="py-2 px-4 border-b">Ends On</th>
                <th className="py-2 px-4 border-b">Availability Period</th>
                <th className="py-2 px-4 border-b">{`Day(s)`}</th>
            ...
            <tbody>
              {bookings.map((booking) => (
                <tr key={booking._id}>
                  ...
                  <td className="py-3 px-4 border-b">
                    {booking.daysOfWeek.map((day) => daysOfWeekMap[day]).join(", ")}
                  </td>
